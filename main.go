...
var (
	...
	tlsInsecure = flag.Bool("tlsInsecure", false, "Accept insecure server TLS certificate.")
...
func generateConfig() (*core.Config, error) {
	...
	if *tlsEnabled {
		tlsConfig := tls.Config{ServerName: *host, AllowInsecure: *tlsInsecure}
		if *server {
